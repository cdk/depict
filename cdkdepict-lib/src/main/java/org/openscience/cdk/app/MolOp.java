...
public class MolOp {
    ...
    private static void neutralize(Set<IAtom> ringAtoms, IAtom metal, IAtom attach) {
        int chargeOnRing = 0;
        for (IAtom atom : ringAtoms) {
            if (atom.equals(attach))
                continue;
            if (atom.getFormalCharge() != null)
                chargeOnRing += atom.getFormalCharge();
            atom.setIsAromatic(true);
            for (IBond bond : atom.bonds()) {
                if (ringAtoms.contains(bond.getOther(atom))) {
                    bond.setIsAromatic(true);
                    bond.setOrder(IBond.Order.UNSET);
                }
            }
        }
        if (metal.getFormalCharge() != null &&
            metal.getFormalCharge() >= -chargeOnRing) {
    ...
    static void setMulticenterStyle(IAtomContainer mol,
                                    MulticenterStyle style) {
